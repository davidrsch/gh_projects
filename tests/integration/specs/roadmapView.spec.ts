        ...
        for (const view of roadmapViews) {
            try {
                await runTestStep(context, `Roadmap View: ${view.name}`, 'Verify roadmap view renders timeline and bars.', async () => {
                    ...
                    // Wait for roadmap to be ready (timeline present and no loading indicator)
                    const waitRes = await sendTestCommand(panel, 'test:waitForRoadmapReady', { timeout: 45000 });
                    if (!waitRes || waitRes.success !== true) {
                        throw new Error('Roadmap did not become ready: ' + (waitRes && (waitRes.error || JSON.stringify(waitRes.debug))));
                    }

                    await snap(page, screenshots, report, `roadmap-${view.name}-loaded`);

                    const roadmapInfo = waitRes; // use the readiness payload
