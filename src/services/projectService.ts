...
import { ProjectEntry, ProjectView, RepoItem, ParsedRepoEntry } from "../lib/types";
...
export class ProjectService {
    ...
    public async loadProjects(workspaceRoot: string): Promise<ProjectEntry[]> {
        ...
        const maxDepth = vscode.workspace.getConfiguration("ghProjects").get<number>("maxDepth", 4);
        let repos: any[] = [];
        try {
            repos = await findGitRepos(workspaceRoot, maxDepth);
            logger.debug(`loadProjects: findGitRepos returned ${repos?.length ?? 0} repos`);
        } catch (e) {
        ...
        const maxConcurrency = vscode.workspace.getConfiguration("ghProjects").get<number>("maxConcurrency", 4);
        ...
        try {
            ...
            logger.debug(`[ghProjects] Project queries result: ${projectQueries?.length ?? 0}`);
        } catch (e) {
        ...
        if (!projectQueries || projectQueries.length === 0) {
            logger.info("No project query results for discovered repos", { reposCount: items.length });
            return [];
        }
    ...
    private async getProjectsForReposArray(arr: RepoItem[]): Promise<ParsedRepoEntry[]> {
        ...
        for (const item of arr || []) {
            ...
            if (Array.isArray(remotes) && remotes.length > 0) {
                for (const r of remotes) {
                    const url = (r && (typeof r === "string" ? r : (r as any).url)) || (r as any);
    ...
    private runCmd(cmd: string, cwd?: string): Promise<{ stdout: string; stderr: string }> {
        ...
        return new Promise<{ stdout: string; stderr: string }>((resolve, reject) => {
            execFile("git", parts, { cwd, maxBuffer: 10 * 1024 * 1024 }, (err, stdout, stderr) => {
